...
train_label_raw=pd.read_table('./train_validation_test/train_label_onehot.txt',header=None)
validation_label_raw=pd.read_table('./train_validation_test/validation_label_onehot.txt',header=None)
test_label_raw=pd.read_table('./train_validation_test/test_label_onehot.txt',header=None)
...
train_turn=1                                                                   #是否进行模型训练
pretdict_turn=0                                                                #是否进行分类预测
...
def train_predict(train_data,train_label,validation_data,validation_label,test_data,test_label):
    ...
    if pretdict_turn==1:
        test_data_raw=pd.read_table('./train_validation_test/feature_vector.txt',header=None)
        test_data=test_data_raw.values
        saver=tf.train.Saver()
        with tf.Session() as sess:   
            saver.restore(sess,"./path/to/model/model.ckpt") 
            prediction=sess.run(average_y,feed_dict={x:test_data})
            prediction_values=sess.run(tf.argmax(prediction,1)) 
            np.savetxt('./train_validation_test/prediction_values.txt',prediction_values,fmt='%s',newline='\n')  
            np.savetxt('./train_validation_test/prediction.txt',prediction,fmt='%s',newline='\n')  
            
        
def main(argv=None):
    train_predict(train_data,train_label,validation_data,validation_label,test_data,test_label)
